          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                {loadingWire && <RefreshCw className="w-3 h-3 text-white/30 animate-spin" />}
              </div>
              ...
              {loadingWire ? (
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
                  <p className="text-sm text-white/50">Wire details not configured yet.</p>
                  <p className="text-xs text-white/30 mt-1">Please contact <a href="mailto:billing@peakestimator.com" className="underline text-copper">billing@peakestimator.com</a> to get payment instructions.</p>
                </div>
              )}

              {wireDetails?.wire_instructions && (
                <p className="text-[11px] text-white/30 mt-2 leading-relaxed">{wireDetails.wire_instructions}</p>
              )}
              {!wireDetails?.wire_instructions && wireRows.length > 0 && (
                <p className="text-[11px] text-white/30 mt-2 leading-relaxed">International transfers: include SWIFT code. Processing typically takes 1–3 business days. Your account will be activated within 24 hours of confirmed receipt.</p>
              )}
            </div>
              ...
              {wireSubmitted ? (
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {submittingWire ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Send className="w-3.5 h-3.5" />}
