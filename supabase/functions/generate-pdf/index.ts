...
serve(async (req) => {
  ...
  try {
    ...
    // Build HTML for PDF-like output as SVG-embedded HTML (simplified)
    // Since full PDFKit in Deno is complex, we return structured HTML the browser can print
    const html = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8" />
<title>ZenBid Proposal - ${project.name}</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .proposal-label { color: #4F46E5; font-size: 26px; font-weight: 700; text-align: right; }
  ...
  .total-row.final { background: #4F46E5; border-radius: 10px; margin-top: 8px; }
  ...
  .sig-block { display: flex; gap: 40px; padding: 24px 0; border-top: 1px solid #E2E8F0; margin-top: 16px; }
  .sig-col { flex: 1; }
  .sig-label { font-size: 9px; font-weight: 700; text-transform: uppercase; color: #94A3B8; letter-spacing: 0.05em; margin-bottom: 40px; }
  .sig-line { border-bottom: 2px solid #CBD5E1; margin-bottom: 8px; }
  .sig-sub { font-size: 10px; color: #94A3B8; }
  .footer { background: #0F172A; padding: 16px 48px; text-align: center; color: #64748B; font-size: 10px; }
...
<div class="footer">${project.company_name || "ZenBid Pro"} · Generated by ZenBid Pro · ${new Date().getFullYear()}</div>
